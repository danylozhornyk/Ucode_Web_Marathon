...
export const FavoritePosts: React.FC<UserProps> = ({ currentUser }) => {
    ...
    const fetchPostsWithComments = async () => {
        setLoading(true);
        try {
            const responsePaginated = await FavoritePostService.getFavorites(
                {
            ...
            const responseAll = await FavoritePostService.getFavorites(
                {
                    categoryIds: selectedCategories,
                    startDate,
                    endDate,
            ...
            const activePosts = responsePaginated.data.filter((post) => post.status !== 'inactive');
            const activePostsAllLength = responseAll.data.filter((post) => post.status !== 'inactive').length;
